# This Makefile is for the Net::Nessus::XMLRPC extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Net/Nessus/XMLRPC.pm]
#     AUTHOR => q[Vlatko Kosturjak <kost@linux.hr>]
#     NAME => q[Net::Nessus::XMLRPC]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0] }
#     VERSION_FROM => q[lib/Net/Nessus/XMLRPC.pm]
#     clean => { FILES=>q[Net-Nessus-XMLRPC-*] }
...
MAKE = make
FULLEXT = Net/Nessus/XMLRPC
BASEEXT = XMLRPC
PARENT_NAME = Net::Nessus
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Net/Nessus/XMLRPC.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Net/Nessus
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Net/Nessus
...
TO_INST_PM = lib/Net/Nessus/XMLRPC.pm

PM_TO_BLIB = lib/Net/Nessus/XMLRPC.pm \
...
DISTNAME = Net-Nessus-XMLRPC
DISTVNAME = Net-Nessus-XMLRPC-0.01
...
manifypods : pure_all  \
	lib/Net/Nessus/XMLRPC.pm
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/Net/Nessus/XMLRPC.pm $(INST_MAN3DIR)/Net::Nessus::XMLRPC.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:                Net-Nessus-XMLRPC' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:             0.01' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:            Communicate with Nessus scanner(v4.2+) via XMLRPC' >> META_new.yml
	$(NOECHO) $(ECHO) 'license:             ~' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:              ' >> META_new.yml
	$(NOECHO) $(ECHO) '    - Vlatko Kosturjak <kost@linux.hr>' >> META_new.yml
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Communicate with Nessus scanner(v4.2+) via XMLRPC</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Vlatko Kosturjak &lt;kost@linux.hr&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  lib/Net/Nessus/XMLRPC.pm blib/lib/Net/Nessus/XMLRPC.pm 
